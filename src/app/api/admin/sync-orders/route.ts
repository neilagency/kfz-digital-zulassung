 ...
 * Syncs recent WP orders to local DB.
 * - Imports any missing orders
 * - Updates status of existing orders
 * - Generates invoices for new orders
 */
export async function POST(request: NextRequest) {
  ...
  try {
    // Fetch last 50 WP orders (covers recent activity)
    ...
    for (const wpOrder of wpOrders) {
      ...
      try {
        ...
        console.error(`Failed to import WP order #${wpOrder.id}:`, err);
      }
    }
